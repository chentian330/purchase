    ...
    with st.container():
        ...
        with col2:
            ...
            if st.button("⬅️ 返回", disabled=back_disabled):
                ...
                # 检查是否在历月对比的子页面
                if st.session_state['current_page'] == 'history_compare' and st.session_state.get('history_compare_subpage') is not None:
                    # 从子页面返回到历月对比主页
    ...
    if stats_df is not None and name_col is not None:
        ...
        if not employees_df.empty:
            ...
            with left_col:
                ...
                try:
                    target_small_orders = int(employee_data['目标小单数']) if '目标小单数' in employee_data and pd.notna(employee_data['目标小单数']) else 0
                except:
                ...
                try:
                    target_amount = float(employee_data['本月采购目标额']) if '本月采购目标额' in employee_data and pd.notna(employee_data['本月采购目标额']) else 0
            ...
            with right_col:
                ...
                try:
                    if '目标小单数' in employee_data and pd.notna(employee_data['目标小单数']) and float(employee_data['目标小单数']) > 0:
                ...
                try:
                    ...
                    elif '本月采购目标额' in employee_data and pd.notna(employee_data['本月采购目标额']) and float(employee_data['本月采购目标额']) > 0:
    ...
    if delivery_df is not None and name_col is not None:
        ...
        if not employees_df.empty:
            ...
            with left_col:
                ...
                # 提取数据值
                try:
                    effective_delivery = float(employee_data['有效拿货量']) if pd.notna(employee_data['有效拿货量']) else 0
                    effective_delivery_str = str(int(effective_delivery))
                except:
                    effective_delivery_str = "数据缺失"
                
                try:
                    target_delivery = float(employee_data['目标拿货量']) if '目标拿货量' in employee_data and pd.notna(employee_data['目标拿货量']) else 0
            ...
            with right_col:
                ...
                delivery_progress = 0
                try:
                    if '目标拿货量' in employee_data and pd.notna(employee_data['目标拿货量']) and float(employee_data['目标拿货量']) > 0:
...
def show_history_purchase_compare():
    ...
    try:
        purchase_data.sort(key=lambda x: (
            ...
            int(x['month_label'].split('年')[1].split('月')[0]) if '年' in x['month_label'] and '月' in x['month_label'] else 0
        ))
    except:
        # 如果排序失败，保持原顺序
        pass
            ...
            if name_col and '合计' in stats_df[name_col].values and '小单数' in stats_df.columns:
                ...
                if not total_row.empty:
                    try:
                        ...
                        # 处理可能的字符串格式
                        if isinstance(orders_value, str) and orders_value.strip() and orders_value.replace('.', '', 1).isdigit():
                        ...
                        if '目标小单数' in row:
                            try:
                                ...
                                if pd.notna(target_orders_value):
                                    if isinstance(target_orders_value, str) and target_orders_value.strip() and target_orders_value.replace('.', '', 1).isdigit():
                                        target_orders_value = float(target_orders_value)
                                    employee_data[emp_name][month_label]['target_small_orders'] = float(target_orders_value)
                            except:
                                pass
...
def show_history_delivery_compare():
    ...
    if not delivery_data:
        show_warning_card("未能从上传的文件中提取到有效的拿货量数据，请确保Excel文件包含'本月拿货统计'工作表和'有效拿货量'列")
        return
    ...
    with col2:
        ...
        fig_completion.update_yaxes(
            ...
            tickformat=".1f",
            tickfont=dict(size=10),
            range=[0, max(120, max([max([item['completion_rate'] for item in employee_data[emp]]) for emp in selected_employees if emp in employee_data]) * 1.1)]
        )
